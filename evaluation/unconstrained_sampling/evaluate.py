...
def keyword_count(generations_df):
    pass
...
def sentiment_classify_own2(generations_df, sentiment_file=None):
    ...
    model_path="/projects/tir5/users/sachink/embed-style-transfer/models/roberta-base-textattack-very-sst-2-with-gpt2-large-embeds/checkpoint_best/"
    config = AutoConfig.from_pretrained(model_path)
    tokenizer = AutoTokenizer.from_pretrained(model_path)
...
def toxicity_score(generations_df, perspective_file, perspective_rate_limit=5):
    ...
    API_KEY="AIzaSyBRDVrYiuvaJbePftke-YvGYMq0KXaJ8DE"
    ...
    for i, row in tqdm(generations_df.iterrows(), total=len(generations_df.index), desc='Evaluating toxicity via perspective API'):
        ...
        while not_done.sum() > 0:
            ...
            for j, text in enumerate(generations):
                ...
                batch_request.add(client.comments().analyze(body=analyze_request), callback=response_callback, request_id=f"gen-{i}-{j}")
...
def main(generations_file, output_file, metrics):
